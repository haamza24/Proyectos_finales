...
start:
		...
		LOAD		s0,rxreg	;guardo lo que he recebido en el s0, porque rxreg se machaca con la resta (propiedad sub)
...
DATO:	
		...
		CALL		recibe				; Mete el # c que me permite ir desde el estado CLAVE al estado de CONTROL
		OUTPUT		rxreg,44
		LOAD		txreg,rxreg		 
		CALL		transmite  
		
		
		CALL		recibe				; Mete o un + o un - un 1 o 2
		OUTPUT		rxreg,44
		LOAD		txreg,rxreg		 
		CALL		transmite  
		...
		INPUT		txreg,50			;quiero que se imprima por pantalla el resultado final 
		TOMY 		txreg
		CALL		transmite
...
clave:						; Aqui llegamos si hemos recibido algo distinto de *
		
		SUB		s0,72		;la resta se hace con el valor que se metio al principio de todo (por estar aqui es qu es!=r)
		JUMP		NZ,clave
							;la rutina de abajo se ejecuta si s0 es r
